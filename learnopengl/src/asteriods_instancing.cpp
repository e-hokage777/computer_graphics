...
int main()
{
    ...
    Shader instanceShader("shaders/asteriods_instancing.vs", "shaders/fragment.fs");
    ...
    unsigned int numAsteriods = 100000;
    float asteriodBeltRadius = 20.0f;
    ...
    {
        float angle = ((float)i / (float)numAsteriods) * 2 * 3.142f;
        float xPos = glm::cos(angle) * asteriodBeltRadius;
        float zPos = glm::sin(angle) * asteriodBeltRadius;

        // randoms
        float xOffset = rand() % 10 - 5;
        float yOffset = rand() % 10 - 5;
        float zOffset = rand() % 10 - 5;
        ...
        asteroidTransforms.push_back(transformMat);

        // instanceTransforms.push_back(InstanceTransform{glm::vec3(xPos + xOffset, yOffset, zPos + zOffset), glm::vec3(scale), rotation});
    }

    ModelInstance asteriod = ModelInstance("assets/rock/rock.obj", asteroidTransforms.size(), asteroidTransforms);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // for (int i = 0; i < asteroidTransforms.size(); ++i)
        // {

        //     shader.uniformMat4("model", asteroidTransforms[i]);
        //     asteriod.Draw(shader);
        // }

        instanceShader.use();
        instanceShader.uniformMat4("projection", projectionMat);
        instanceShader.uniformMat4("view", camera.getMatrix());
        instanceShader.uniformMat4("model", modelMat);
        asteriod.Draw(instanceShader);
